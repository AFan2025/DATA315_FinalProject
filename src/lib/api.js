 ...
 * @param {{ k?: number, threshold?: number, signal?: AbortSignal }} [opts]
 */
export async function search(conceptWeights, { k = 20, threshold = 0.25, signal } = {}) {
  const res = await fetch(`${API_BASE}/search?k=${k}&threshold=${threshold}`, {
    method: 'POST',
